...
def draw_tracks(image, tracks):
    ...
    for trk in tracks:
        ...
        tracks_all.append((trk_id, xcentroid, ycentroid))
        text = "ID {}".format(trk_id)

        cv.putText(
            img = image,
            text = text,
            org = (xcentroid - 10, ycentroid - 10),
            fontFace = cv.FONT_HERSHEY_SIMPLEX,
            fontScale = 0.5,
            color = (255, 255, 255),
        )
        # print("fasdfadsfasdfasdfasdfasdf")
        cv.circle(image, (xcentroid, ycentroid), 4, (255, 255, 255), -1)

    return image, tracks_all
...
if __name__ == "__main__":
    ...
    dict2jsonfile(
        data,
        "../../examples/pretrained_models/caffemodel_weights/ssd_mobilenet_caffe_names.json",
    )
