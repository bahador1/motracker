...
class Detector:
    ...
    def __init__(
        self, object_names, confidence_threshold, nms_threshold, draw_bboxes=True
    ):
        ...
        if draw_bboxes:
            self.bbox_colors = {
                key: np.random.randint(0, 255, size=(3,)).tolist()
    ...
    def detect(self, image):
        ...
        if len(bboxes):
            ...
            indices = cv.dnn.NMSBoxes(
                bboxes, confidences, self.confidence_threshold, self.nms_threshold
            ).flatten()
            return (
                np.array(bboxes)[indices, :],
    ...
    def draw_bboxes(self, image, bboxes, confidences, class_ids):
        ...
        for bb, conf, cid in zip(bboxes, confidences, class_ids):
            ...
            cv.putText(
                image, label, (bb[0], y_label), cv.FONT_HERSHEY_SIMPLEX, 0.5, clr, 2
            )
